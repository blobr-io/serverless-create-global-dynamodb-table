 ...
 */
const checkStackCreateUpdateStatus = async function checkStackCreateUpdateStatus(
  cfn,
  stackName,
  region,
  cli
  ...
  cli.consoleLog(
    `CreateGlobalTable: ${chalk.yellow(
      `Checking cloudformation stack ${stackName} status in ${region}...`
    )}`
  );
  let status;
  let dotPrinted = false;
  while (TRUE) {
    const resp = await cfn
      .describeStacks({
        StackName: stackName,
      })
      .promise();
  ...
  if (status.includes('ROLLBACK')) {
    cli.consoleLog(
      `CreateGlobalTable: ${chalk.red(`Failed to create/update the stack ${stackName} in ${region}... \n
 ...
 * @param {Object} cli Serverless cli object
 */
const createUpdateCfnStack = async function createUpdateCfnStack(
  cfn,
  template,
  stackName,
  region,
  ...
  cli.consoleLog(
    `CreateGlobalTable: ${chalk.yellow(
      `Creating/Updating cloudformation stack ${stackName} in ${region}...`
    )}`
  );
  let update = false;
  try {
  ...
  const stackSuccess = await module.exports.checkStackCreateUpdateStatus(
    cfn,
    stackName,
    region,
    cli
  ...
  if (stackSuccess) {
    cli.consoleLog(
      `CreateGlobalTable: ${chalk.yellow(
        `Cloudformation stack ${stackName} successfully created/updated in ${region}...`
      )}`
    );
  }
};
...
const createNewTableAndSetScalingPolicy = async function createNewTableAndSetScalingPolicy(
  ...
  cli.consoleLog(
    `CreateGlobalTable: ${chalk.yellow(
      `Creating new table ${tableName} in ${region} region...`
    )}`
  );
  try {
    ...
    cli.consoleLog(
      `CreateGlobalTable: ${chalk.yellow(
        `Created new table ${tableName} in ${region} region...`
      )}`
    );
    if (scalingPolicies.length) {
      cli.consoleLog(
        `CreateGlobalTable: ${chalk.yellow('Adding auto scaling setting')}`
      );
      const writePolicy = scalingPolicies.find(
        (p) => p.PolicyName === WRITEAUOTSCALINGPOLICY
      );
      const readPolicy = scalingPolicies.find(
        (p) => p.PolicyName === READAUOTSCALINGPOLICY
      );
      if (writePolicy) {
        const regScalableTargetParams = {
          ...
          MinCapacity:
            createTableParams.ProvisionedThroughput.WriteCapacityUnits,
          MaxCapacity:
            createTableParams.ProvisionedThroughput.WriteCapacityUnits,
        };
        await appAutoScaling
          .registerScalableTarget(regScalableTargetParams)
          .promise();
        cli.consoleLog(
          `CreateGlobalTable: ${chalk.yellow(
            'Registered Write Scalable target'
          )}`
        );
        const scalingParam = {
          ...
          TargetTrackingScalingPolicyConfiguration: {
            ...writePolicy.TargetTrackingScalingPolicyConfiguration,
          },
        };
        await appAutoScaling.putScalingPolicy(scalingParam).promise();
        cli.consoleLog(
          `CreateGlobalTable: ${chalk.yellow('Added Write Scaling policy')}`
        );
      }
      if (readPolicy) {
        const regScalableTargetParams = {
          ...
          ServiceNamespace: 'dynamodb',
          MinCapacity:
            createTableParams.ProvisionedThroughput.ReadCapacityUnits,
          MaxCapacity:
            createTableParams.ProvisionedThroughput.ReadCapacityUnits,
        };
        await appAutoScaling
        ...
        cli.consoleLog(
          `CreateGlobalTable: ${chalk.yellow(
            'Registered Read Scalable target'
          )}`
        );
        const scalingParam = {
          ...
          PolicyType: 'TargetTrackingScaling',
          TargetTrackingScalingPolicyConfiguration: {
            ...readPolicy.TargetTrackingScalingPolicyConfiguration,
          },
        };
        await appAutoScaling.putScalingPolicy(scalingParam).promise();
        cli.consoleLog(
          `CreateGlobalTable: ${chalk.yellow('Added Read Scaling policy')}`
        );
      }
      cli.consoleLog(
        `CreateGlobalTable: ${chalk.yellow(
          'Auto scaling policy added successfully'
        )}`
      );
    }
  } catch (error) {
    if (error.code === 'ResourceInUseException') {
      cli.consoleLog(
        `CreateGlobalTable: ${chalk.yellow(
          `Table ${tableName} already exists in the region ${region}`
        )}`
      );
  ...
  try {
    ...
    if (version === 'v2') {
      ...
      if (resp.Table.Replicas !== undefined) {
        regionsGlobalTableExists = resp.Table.Replicas.map(
          (rg) => rg.RegionName
        );
      }
      missingRegions = newRegions.filter(
        (r) => !regionsGlobalTableExists.includes(r)
      );
    } else {
      let resp = await dynamodb
        .describeGlobalTable({ GlobalTableName: tableName })
        .promise();
      regionsGlobalTableExists = resp.GlobalTableDescription.ReplicationGroup.map(
        (rg) => rg.RegionName
      );
      missingRegions = [region]
        .concat(newRegions)
        .filter((r) => !regionsGlobalTableExists.includes(r));
    }

    if (missingRegions.length === 0) {
      cli.consoleLog(
        `CreateGlobalTable: ${chalk.yellow(
          `Global table ${tableName} already exists in all the specified regions. Skipping creation...`
        )}`
      );
    ...
    cli.consoleLog(
      `CreateGlobalTable: ${chalk.yellow("Global table doesn't exist...")}`
    );
...
const createGlobalTable = async function createGlobalTable(
  ...
  const {
    missingRegions: regionsToUpdate,
    addingNewRegions,
  } = await module.exports.getRegionsToCreateGlobalTablesIn(
    dynamodb,
    region,
    newRegions,
    tableName,
  ...
  if (!regionsToUpdate.length) {
    cli.consoleLog(
      `CreateGlobalTable: ${chalk.yellow(
        `Global table setup already in place.`
      )}`
    );
    return;
  }
  ...
  if (!createStack && version !== 'v2') {
    const tableDef = await dynamodb
      .describeTable({ TableName: tableName })
      .promise();
    const {
      ReadCapacityUnits,
      WriteCapacityUnits,
    } = tableDef.Table.ProvisionedThroughput;
    ...
    const tags = await dynamodb
      .listTagsOfResource({ ResourceArn: tableDef.Table.TableArn })
      .promise();
    createTableParams.Tags = tags.Tags;

    const describeScalingPoliciesResp = await appAutoScaling
    ...
    await Promise.all(
      regionsToUpdate.map((r) => {
        ...
        return module.exports.createNewTableAndSetScalingPolicy(
          ...
          ddb,
          createTableParams,
          describeScalingPoliciesResp.ScalingPolicies,
          tableName,
          r,
          cli
        );
  ...
  if (version === 'v2') {
    await module.exports.createGlobalTableV2(
      dynamodb,
      tableName,
      regionsToUpdate,
      cli
    ...
    await module.exports.createGlobalTableV1(
      dynamodb,
      tableName,
      region,
      regionsToUpdate,
...
const createGlobalTableV1 = async function createGlobalTableV1(
  dynamodb,
  tableName,
  region,
  regionsToUpdate,
  ...
  cli.consoleLog(
    `CreateGlobalTable: ${chalk.yellow(
      `Create global table setup (Version 2017.11.29) for ${tableName}...`
    )}`
  );
  if (!addingNewRegions) {
    ...
    regionsToUpdate.forEach((r) =>
      replicaUpdates.push({ Create: { RegionName: r } })
    );
    const param = {
      GlobalTableName: tableName,
  ...
  cli.consoleLog(
    `CreateGlobalTable: ${chalk.yellow(
      `Created global table setup (Version 2017.11.29) for ${tableName}...`
    )}`
  );
};

const createGlobalTableV2 = async function createGlobalTableV2(
  dynamodb,
  tableName,
  regionsToUpdate,
  cli
  ...
  cli.consoleLog(
    `CreateGlobalTable: ${chalk.yellow(
      `Create global table setup (Version 2019.11.21) for ${tableName}...`
    )}`
  );
  for (const region of regionsToUpdate) {
    ...
    cli.consoleLog(
      `CreateGlobalTable: ${chalk.yellow(
        `Start creating a replica for ${tableName} in ${region}`
      )}`
    );
    ...
    cli.consoleLog(
      `CreateGlobalTable: ${chalk.yellow(
        `The replica for ${tableName} in ${region} has been created successfully`
      )}`
    );
  }
  cli.consoleLog(
    `CreateGlobalTable: ${chalk.yellow(
      `The global table setup (Version 2019.11.21) for ${tableName} has been created successfully`
    )}`
  );
  ...
  // res = await dynamodb.waitFor('tableExists', {TableName: tableName}).promise(); // it's gonna wait for "Active" status
  // console.log(res)
  // cli.consoleLog(`CreateGlobalTable: ${chalk.yellow(`Start creating a replica for ${tableName} in us-east-1`)}`)
  // res = await dynamodb.updateTable(params).promise();
  // console.log(res)
  // await sleep(1000);
  // res = await dynamodb.waitFor('tableExists', {TableName: tableName}).promise(); // it's gonna wait for "Active" status
  // console.log(res)
  ...
  // res = await dynamodb.waitFor('tableExists', {TableName: tableName}).promise(); // it's gonna wait for "Active" status
  // console.log(res)
  // cli.consoleLog(`CreateGlobalTable: ${chalk.yellow(`Start creating a replica for ${tableName} in us-west-1`)}`)
  // res = await dynamodb.updateTable(params).promise();
  // await sleep(1000);
  // console.log(res)
  // res = await dynamodb.waitFor('tableExists', {TableName: tableName}).promise(); // it's gonna wait for "Active" status
 ...
 */
const getTableNamesFromStack = async function getTableNamesFromStack(
  cfn,
  stackName
) {
  const resp = await cfn
    ...
    .promise();
  const tablesInStack = resp.StackResources.filter(
 ...
 */
const createGlobalDynamodbTable = async function createGlobalDynamodbTable(
  serverless
) {
  try {
    serverless.cli.consoleLog(
      `CreateGlobalTable: ${chalk.yellow(
        'Starting setting up global tables...'
      )}`
    );
    ...
    if (globalTablesOptions.createStack === false) {
      ...
      for (let newRegion of globalTablesOptions.regions) {
        ...
        await module.exports.createUpdateCfnStack(
          cfn,
          cfnTemplate,
          stackName,
          newRegion,
    ...
    serverless.cli.consoleLog(
      `CreateGlobalTable: ${chalk.red(
        `Failed to setup global table. Error ${error.message || error}`
      )}`
    );
  }
};
